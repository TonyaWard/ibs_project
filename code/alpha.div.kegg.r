...
mc$shannon_kegg <- alpha_divC$obs_species
mc$shannon_kegg <- alpha_divC$simpson
...
figure2 <- ggplot(working_alpha[working_alpha$ID_on_tube %in% names(which(working_alpha2>=3)),]) +
  ...
  geom_jitter(size=1.5, data=m[m$Flare == "Flare" & !is.na(m$Flare),], aes(y=shannon_kegg, x= Flare_timepoint, color= Cohort), width = 0.25)
