...
for(i in 1:length(test.ixs)){
  ...
  sink(paste("modules/machine_learning/", names(test.ixs)[i], ".txt", sep=""))
  ...
  pdf(paste('modules/machine_learning/RF_importance', names(test.ixs)[i], '.pdf', sep=""), width=11, height=3)
  plot1 <- ggplot(microbes, aes(x=names, y= Importance)) +
    geom_bar(stat="identity", fill="#9ba11e") +
    coord_flip() +
