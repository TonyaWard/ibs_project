#Calculate Beta Div
beta_div <- as.matrix(vegdist(modules, method = "bray"))
write.csv(beta_div, file="modules/beta_div/IBS_BrayCurtis.csv", quote=F, row.names=T)
...
pdf("modules/beta_div/PCOAs.pdf", height=8, width=9)
plot(PCOAs)
dev.off()
...
#make stats file
file_name <- paste("modules/beta_div/Beta_Stats.txt", sep='')
sink(file_name)
sink()
...
for(n in 1:(length(test.ixs)-1)){
  for(i in (n+1):length(test.ixs)){
    ...
    name1 <- paste("modules/beta_div/", name1, ".pdf", sep='')
...
pdf("modules/beta_div/cohort_flares.pdf", width=10, height=4)
plot(together)
dev.off()
...
pdf("modules/beta_div/patients_flares.pdf", width=10, height=9)
plot(together)
dev.off()
...
#Calculate Beta Div
beta_div <- as.matrix(vegdist(modulesc, method = "bray"))
write.csv(beta_div, file="modules/beta_div/IBS_BrayCurtis.collapsed.csv", quote=F, row.names=T)
...
plot3 <- ggplot(PCOA) +
  ...
  theme(legend.title=element_blank())
PCOAs <- plot_grid(plot1, plot2, plot3, ncol=2, nrow=2)
pdf("modules/beta_div/PCOAs_Collapsed.pdf", height=8, width=9)
plot(PCOAs)
dev.off()
