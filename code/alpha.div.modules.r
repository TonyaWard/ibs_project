...
alpha_div <- as.data.frame(diversity(modules, index="shannon"))
colnames(alpha_div) <- "shannon"
alpha_div$simpson <- diversity(modules, index="simpson")
alpha_div$obs_species <- rowSums(modules > 0)
alpha_div <- alpha_div[rownames(m),]
m$shannon_modules <- alpha_div$shannon
m$observed_species_modules <- alpha_div$obs_species
m$simpson_modules <- alpha_div$simpson

#Alpha Div for collapsed
alpha_divC <- as.data.frame(diversity(modulesc, index="shannon"))
colnames(alpha_divC) <- "shannon"
alpha_divC$simpson <- diversity(modulesc, index="simpson")
alpha_divC$obs_species <- rowSums(modulesc > 0)
alpha_divC <- alpha_divC[rownames(mc),]
mc$shannon_modules <- alpha_divC$shannon
mc$shannon_modules <- alpha_divC$obs_species
mc$shannon_modules <- alpha_divC$simpson
...
#write stats to file
file_name <- "modules/alpha_div/alpha_div.txt"
...
file_path <- "modules/alpha_div/alpha_div.pdf"
...
working_alpha <- melt(m, id.vars = c("SampleID", "Cohort", "Timepoint", "ID_on_tube"), measure.vars = c("shannon_modules"))
...
file_path <- "modules/alpha_div/alpha_div_time.pdf"
pdf(file_path, height=4,width=6)
plot(figure)
dev.off()
...
file_path <- "modules/alpha_div/Alpha_Variance.pdf"
pdf(file_path, height=4,width=6)
