...
dotu <- as.matrix(vegdist(modules))
...
for(i in 1:length(test.xs)){
    ...
    for(j in 1:length(test.ixs)){
        ...
        if(tt$p.value < ALPHA) {
          sink("modules/beta_div/variability.txt", append=T)
          ...
          pdf(sprintf('modules/beta_div/variability_%s_%s.pdf',x.name, gsub(' ','_',test.name)),width=4,height=4, useDingbats=FALSE)
          print(plot1)
          dev.off()
        }
    }
