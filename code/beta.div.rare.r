#Calculate Beta Div
beta_div <- as.matrix(vegdist(x.raw.r, method = "bray"))
write.csv(beta_div, file="beta_div/rare/IBS_BrayCurtis.csv", quote=F, row.names=T)
...
pdf("beta_div/rare/PCOAs.pdf", height=8, width=9)
plot(PCOAs)
dev.off()
...
#make stats file
file_name <- paste("beta_div/rare/Beta_Stats.txt", sep='')
sink(file_name)
sink()
...
for(n in 1:(length(test.ixs)-1)){
  for(i in (n+1):length(test.ixs)){
    ...
    name1 <- paste("beta_div/rare/", name1, ".pdf", sep='')
...
pdf("beta_div/rare/cohort_flares.pdf", width=10, height=4)
plot(together)
dev.off()
...
pdf("beta_div/rare/patients_flares.pdf", width=10, height=9)
plot(together)
dev.off()
...
#Calculate Beta Div
beta_div <- as.matrix(vegdist(xc.r[,which(colMeans(xc.r) > 0.0005)], method = "bray"))
write.csv(beta_div, file="beta_div/rare/IBS_BrayCurtis.collapsed.csv", quote=F, row.names=T)
...
plot3 <- ggplot(PCOA) +
  ...
  theme(legend.title=element_blank())
PCOAs <- plot_grid(plot1, plot2, plot3, ncol=2, nrow=2)
pdf("beta_div/rare/PCOAs_Collapsed.pdf", height=8, width=9)
plot(PCOAs)
dev.off()
